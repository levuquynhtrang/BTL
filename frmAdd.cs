    ...
    public partial class frmAdd : Form
    {
        ...
        private void btnFix_Click(object sender, EventArgs e)
        {
            ...
            int i = int.Parse(INDEX);
            {
                ...
                {
                    if (MessageBox.Show("Bạn có chắc chắn muốn sửa bản ghi hiện thời?", "Xác nhận yêu cầu", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                    {
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            ...
            sql = "select MaMon from tblLopHP where MaLopHP='" + tMaHP + "' or TenLopHP = '" + tLopHP + "'";
            da = new SqlDataAdapter(sql, conn);
            ...
            if (dtLS.Rows.Count == 0)
            {
                ...
                if (MessageBox.Show("Bạn có chắc chắn muốn thêm bản ghi hiện thời?", "Xác nhận yêu cầu", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    sql = "Insert into tblLopHP(MaLopHP,MaMon,TenLopHP,SiSo,GiangVien,PhongHoc,TietHoc,SSHienTai)" +
                    " Values ('" + tMaHP + "','" + tMaMon + "',N'" + tLopHP + "'," +
                      SiSo + ", N'" + tGiangVien + "',N'" + tPhongHoc + "',N'" + tTietHoc + "'," + SSHT + " )";
                    cmd.Connection = conn;
                    cmd.CommandText = sql;
                    cmd.ExecuteNonQuery();
                    NapLai();
            ...
            {
                MessageBox.Show("Tên lớp học phần hoặc mã lớp học phần đã tồn tại!");
            }
        }
    }
}
